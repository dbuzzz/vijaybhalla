...
$("#save_form").on("submit",function(e){
  ...
  $.ajaxSetup({
   ...
   fd.append('desc',editor_content);
	$.ajax({ 
		type:"post",
		url:siteUrl + "/add-testimonial/save",  
		data:fd,
		processData: false, 
        contentType: false, 
		success:function(res){
			if(res.status_code == 200){
                ...
                reset();
                window.location.href = siteUrl + "/add-testimonial";
...
function showDatatable(){
	...
	var table = $('#cat_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: siteUrl+"/add-testimonial/show",
        columns: [ 
            {data: 'DT_RowIndex'},
            {data: 'name', name: 'name'},
...
function deletes(id = ''){
    if (confirm("Are you sure!")) {
        $.ajax({
            url:siteUrl+"/add-testimonial/delete",
            type:'GET',
            data:{id:id},
...
function status(id = "", status = "") {
    $.ajax({
        url: siteUrl +"/add-testimonial/status",
