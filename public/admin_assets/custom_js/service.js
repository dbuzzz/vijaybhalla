	...
	$.ajax({ 
		...
		success:function(res){
			if(res.status_code == 200){
                ...
                reset();
                window.location.href = siteUrl + "/add-Service";
...
function deletes(id = ''){
    if (confirm("Are you sure!")) {
        $.ajax({
            url:siteUrl+"/add-Service/delete",
            type:'GET',
            data:{id:id},
...
function status(id = "", status = "") {
    $.ajax({
        url: siteUrl +"/add-Service/status",
...
function view(id = '') {
    // 
    $.ajax({
        url: siteUrl +"/add-Service/price_calc",
        data: { id: id},
        type: "get",
        dataType: "json",
        success: function (response) {
...
$(".add_field_button").click(function(e){ 
    e.preventDefault();
    var html = '<div class="form-group col-lg-4"> <label for="exampleInputEmail1">Service-Heading</label> <input type="text" class="form-control" name="serviceHeading[]" placeholder="Enter serviceHeading .." value=""> </div> <div class="form-group col-lg-4"> <label for="exampleInputEmail1">Service Sub-Heading</label> <input type="text" class="form-control" name="serviceSubheading[]" placeholder="Enter serviceSubheading .." value=""> </div> <div class="form-group col-lg-3"> <label for="exampleInputEmail1">Service Image</label><span style="color:red">1080 x 1080 px</span> <input type="file" class="form-control" name="serviceimage[]" accept=".jpg,.png,.jpeg"> </div> <div class="col-md-1 mt-4 ap_data"> <button class="remove_field btn btn-danger btn-xs">-</button> </div>';
...
$(".add_field_buttons").click(function(e){ 
    e.preventDefault();
    var htmls = '<div class="form-group col-lg-5"> <label for="exampleInputEmail1">Question</label> <input type="text" class="form-control" name="question[]" placeholder="Enter question .." value=""> </div> <div class="form-group col-lg-5"> <label for="exampleInputEmail1">Answer</label> <input type="text" class="form-control" name="answer[]" placeholder="Enter answer .." value=""> </div> <div class="col-md-1 mt-4 ap_data"> <button class="remove_fields btn btn-danger btn-xs">-</button> </div>';
    $("#appendDatas").before(htmls);
});
