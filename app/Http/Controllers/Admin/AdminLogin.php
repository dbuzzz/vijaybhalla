...
use Validator;
use Auth;
use App\Models\{User,Role,Attribute,Attendance};
...
class AdminLogin extends Controller
{
    ...
    public function attendace_check(request $request) 
    {
         $user= Attendance::where(['date'=>date('Y-m-d'),'attendance'=>1,'user_id'=>Auth::user()->id])->first(['id','created_at']);
         if (!empty($user)) {
    ...
    public function mark_attendance(Request $request){
        ...
        // $user_id = $request->post('user_id');
            $formdata['user_id'] = $request->post('user_id');
            $formdata['added_by'] = $request->post('user_id');
            $formdata['date'] = date("Y-m-d") ;
            $check = Attendance::where(['user_id' =>$request->post('user_id'),'date' => date("Y-m-d")])->get();
            // dd($check);
            if (count($check) == 0) {
                // code...
            $last_id = Attendance::insertGetId($formdata);

                if($last_id){
                    // session(['attendance_status'=>true]);
