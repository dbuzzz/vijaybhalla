...
class ServicesController extends Controller
{
    ...
    {
        if (!empty($request->id)) {
            $data['data'] = Services::where(['id'=>$request->id])->first();
        }
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['subHeading'] = $request->subheading;
            $formdata['description'] = $request->desc;
            $formdata['description2'] = $request->desc2;
            $formdata['serviceHeading'] = implode('$HARSH$', $request->serviceHeading);
            $formdata['serviceSubheading'] = implode('$HARSH$', $request->serviceSubheading);
            $formdata['question'] = implode('$HARSH$', $request->question);
            $formdata['answer'] = implode('$HARSH$', $request->answer);
 
            
            if(!empty($request->file('smallIcon'))){
                    $img = $request->file('smallIcon');
                    $formdata['smallIcon'] = url('uploads').'/'.uniqid().'-'.$img->getClientOriginalName(); 
                    $request->file('smallIcon')->move(public_path('uploads'), $formdata['smallIcon']);
            }
            ...
            if(!empty($request->file('serviceimage'))){
                foreach ($request->file('serviceimage') as $key => $value) {
                    $img = $value;
                    $serviceImage[] = $sImage = url('uploads').'/'.uniqid().'-'.$img->getClientOriginalName(); 
                    $value->move(public_path('uploads'), $sImage);
                }
               $formdata['serviceImage'] = implode('$HARSH$', $serviceImage); 
            }
            

            if (!empty($id)) {
                $formdata['updated_by'] = Auth::user()->id;
                $row = Services::where('id',$id)->update($formdata);
                $row=$id;
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = Services::insertGetId($formdata);
                $msg = 'Data Inserted';
    ...
    public function delete(Request $request){
        ...
        $row = Services::where('id',$request->id)->update($data);
