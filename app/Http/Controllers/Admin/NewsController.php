...
class NewsController extends Controller
{
    ...
    public function save(Request $request){
        ...
        if($validator->passes()) {
            ...
            $formdata['name'] = $request->desc;
         
            if (!empty($id)) {
                $formdata['updated_by'] = Auth::user()->id;
                $row = News::where('id',$id)->update($formdata);
                $row=$id;
                $msg = 'Data Updated';
            }else{
                $formdata['added_by'] = Auth::user()->id;
                $row = News::insertGetId($formdata);
                $msg = 'Data Inserted';
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                    ...
                    ->addColumn('descr', function($row){
                        $btn ="";
                        $btn.= '<span id="hide'.$row->id.'">'.substr($row->name, 0,2).'...';

                        $btn.= '<a href="javascript:void[0]" onclick="show('.$row->id.')">Read</a></span>';

                        $btn.= '<span style="display:none" id="show'.$row->id.'">'.$row->name.'...<a href="javascript:void[0]" onclick="hide('.$row->id.')">Read Less</a></span>';
    ...
    public function delete(Request $request){
        ...
        $row = News::where('id',$request->id)->update($data);
